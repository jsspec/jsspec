...
module.exports = {
  ...
  instance: {
    ...
    findSharedContext(requestedDescription) {
      return this.sharedContexts.find(({ description }) => description === requestedDescription) || this.parent.findSharedContext(requestedDescription);
    },
  },
  global(description, optionsOrBlock, block) {
    if (this.executing) {
      throw new ReferenceError('A shared context can not be defined inside an example');
    }
