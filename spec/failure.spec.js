...
describe("failures", () => {
  ...
  it("fails as expected", () => {
    ...
    expect(result).to.match(
      /\d+\) \[In after hook\] after hook fails with it's own report/
    );
    ...
    expect(result).to.match(
      /\d+\) Error preparation hits the non-filename code for an eval/
    );
    expect(result).to.match(
      /\d+\) Location extraction hits the non-filename code/
    );

    // Bad invocation
    expect(result).to.match(/\d+\) Bad invocation/);
    expect(result).to.include(
      "No shared example named 'a shared example that does not exist' available in this context"
    );
    expect(result).to.include(
      "No shared context named 'a shared context that does not exist' available in this context"
    );
    expect(result).to.include("Threw a string");
    expect(result).to.include("   5\n");
    // subject assignment
    expect(result).to.match(
      /ReferenceError: `subject` is assignable only inside an example block/
    );
  });
});
